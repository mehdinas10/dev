...
class NewPassVC: EnhancedVC {
    ...
    @IBAction func btnSetPass(_ sender: Any) {
        ...
        if edtNewPass.text != nil && edtRepeatPass.text != nil{
            if edtNewPass.text!.count >= 4 && edtRepeatPass.text!.count >= 4 {
                if edtNewPass.text! == edtRepeatPass.text! {
                    ...
                    showToast(message: NSLocalizedString("toast_success_new_pass", comment: ""), controller:self , state:true)
